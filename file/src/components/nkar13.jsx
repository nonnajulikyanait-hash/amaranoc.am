...
export default function Nkar13() {
  const [selectedCurrencyId, setSelectedCurrencyId] = useState(0);

  // Արժույթների տվյալներ և մոտավոր փոխարժեքներ
  const currencies = [
    { id: 0, symbol: '֏', code: 'AMD', rate: 1 },
    { id: 1, symbol: '$', code: 'USD', rate: 388 },
    { id: 2, symbol: '€', code: 'EUR', rate: 415 },
    { id: 3, symbol: '₽', code: 'RUB', rate: 4.2 },
  ];

  // Բազային արժեքները AMD-ով
  const baseDayPrice = 100000;
  const baseNightPrice = 100000;

  const currentCurrency = currencies[selectedCurrencyId];

  // Ընթացիկ գների հաշվարկ ըստ ընտրված արժույթի
  const activeDayPrice = selectedCurrencyId === 0 ? baseDayPrice : Math.round(baseDayPrice / currentCurrency.rate);
  const activeNightPrice = selectedCurrencyId === 0 ? baseNightPrice : Math.round(baseNightPrice / currentCurrency.rate);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between border border-gray-200 rounded-2xl p-4 mb-6 gap-4 shadow-sm">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 text-xl font-black text-gray-900">
              <span className="text-orange-500 text-2xl">📍</span> Սևան
            </div>
          ...
          <div className="flex flex-wrap items-center gap-6 md:gap-8">
            <div>
              ...
              <span className="text-2xl font-black text-orange-500">{activeDayPrice.toLocaleString()}{currentCurrency.symbol}</span>
            </div>
            ...
            <div>
              ...
              <span className="text-2xl font-black text-orange-500">{activeNightPrice.toLocaleString()}{currentCurrency.symbol}</span>
            </div>
            ...
            <div className="flex bg-gray-100 p-1 rounded-full border border-gray-200 gap-1">
              {currencies.map((curr) => (
                <button 
                  ...
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${selectedCurrencyId === curr.id ? 'bg-[#0f172a] text-white' : 'text-gray-600 hover:bg-gray-200'}`}
                >
                  {curr.symbol}
                </button>
              ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 rounded-3xl overflow-hidden mb-8 relative">
          ...
          <div className="md:col-span-6 h-[300px] md:h-[500px] relative group">
            <img 
              src="https://amaranoc.am/_next/image?url=https%3A%2F%2Fapi.amaranoc.am%2F1781688420744--0.6763048347857996image_optimized.webp&w=3840&q=75" 
              alt="Pool Large" 
          ...
          <div className="md:col-span-6 grid grid-cols-2 gap-3 h-[300px] md:h-[500px] relative">
            <img src="https://amaranoc.am/_next/image?url=https%3A%2F%2Fapi.amaranoc.am%2F1781688420763--0.4189431170543112image_optimized.webp&w=3840&q=75" alt="Pool 1" className="w-full h-full object-cover" />
            <img src="https://amaranoc.am/_next/image?url=https%3A%2F%2Fapi.amaranoc.am%2F1781688420765--0.3395723779465194image_optimized.webp&w=3840&q=75" alt="Villa Exterior" className="w-full h-full object-cover" />
            <img src="https://amaranoc.am/_next/image?url=https%3A%2F%2Fapi.amaranoc.am%2F1781688420766--0.6420803785123592image_optimized.webp&w=3840&q=75" alt="A-Frame House" className="w-full h-full object-cover" />
            <div className="relative w-full h-full">
              <img src="https://amaranoc.am/_next/image?url=https%3A%2F%2Fapi.amaranoc.am%2F1781688420767--0.4671263122667739image_optimized.webp&w=3840&q=75" alt="Pool Interior" className="w-full h-full object-cover" />
              {/* Տեսնել բոլորը կոճակ */}
