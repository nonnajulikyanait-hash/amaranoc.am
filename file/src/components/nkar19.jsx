...
export default function Nkar19() {
  const [currency, setCurrency] = useState('AMD');

  // Բազային արժեքները դրամով (50,000) և մոտարկված փոխարժեքները
  const basePriceDay = 50000;
  const basePriceNight = 50000;
  
  const rates = { AMD: 1, USD: 388, EUR: 415, RUB: 4.2 };
  const symbols = { AMD: '֏', USD: '$', EUR: '€', RUB: '₽' };

  // Ընթացիկ գների և նշանի դինամիկ հաշվարկ
  const currentSymbol = symbols[currency];
  const calculatedPriceDay = currency === 'AMD' ? basePriceDay : Math.round(basePriceDay / rates[currency]);
  const calculatedPriceNight = currency === 'AMD' ? basePriceNight : Math.round(basePriceNight / rates[currency]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between border border-gray-200 rounded-2xl p-4 mb-6 gap-4 shadow-sm">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 text-xl font-black text-gray-900">
              <span className="text-orange-500 text-2xl">📍</span> Ծաղկաձոր
            </div>
          ...
          <div className="flex flex-wrap items-center gap-6 md:gap-8">
            <div>
              <span className="block text-xs text-gray-400 font-bold uppercase">Արժեք {currency !== 'AMD' && `(${currency})`}</span>
              <span className="text-2xl font-black text-orange-500">
                {calculatedPriceDay.toLocaleString()}{currentSymbol}
              </span>
            </div>
            ...
            <div>
              ...
              <span className="text-2xl font-black text-orange-500">
                {calculatedPriceNight.toLocaleString()}{currentSymbol}
              </span>
            </div>
            <div className="border-l border-gray-200 h-8 hidden sm:block"></div>
            ...
            <div className="flex bg-gray-100 p-1 rounded-full border border-gray-200 gap-1">
              {[
                ...
                <button 
                  ...
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${currency === item.code ? 'bg-[#0f172a] text-white' : 'text-gray-600 hover:bg-gray-200'}`}
                >
                  {item.sym}
                </button>
              ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 rounded-3xl overflow-hidden mb-8 relative">
          ...
          <div className="md:col-span-6 h-[300px] md:h-[500px] relative group">
            <img 
              src="https://amaranoc.am/_next/image?url=https%3A%2F%2Fapi.amaranoc.am%2F1781421582298--0.8472944485299274image_optimized.webp&w=3840&q=75" 
              alt="Pool Large" 
          ...
          <div className="md:col-span-6 grid grid-cols-2 gap-3 h-[300px] md:h-[500px] relative">
            <img src="https://amaranoc.am/_next/image?url=https%3A%2F%2Fapi.amaranoc.am%2F1781421582318--0.7477492170301931image_optimized.webp&w=3840&q=75" alt="Pool 1" className="w-full h-full object-cover" />
            <img src="https://amaranoc.am/_next/image?url=https%3A%2F%2Fapi.amaranoc.am%2F1781421582359--0.1018303083907004image_optimized.webp&w=3840&q=75" alt="Villa Exterior" className="w-full h-full object-cover" />
            <img src="https://amaranoc.am/_next/image?url=https%3A%2F%2Fapi.amaranoc.am%2F1781421582331--0.046307614245655104image_optimized.webp&w=3840&q=75" alt="A-Frame House" className="w-full h-full object-cover" />
            <div className="relative w-full h-full">
              <img src="https://amaranoc.am/_next/image?url=https%3A%2F%2Fapi.amaranoc.am%2F1781421582344--0.609022043351011image_optimized.webp&w=3840&q=75" alt="Pool Interior" className="w-full h-full object-cover" />
              {/* Տեսնել բոլորը կոճակ */}
        ...
        <div className="w-full h-[350px] md:h-[450px] rounded-3xl overflow-hidden relative shadow-inner mb-12 border border-gray-200">
          ...
          <iframe 
            ...
            allowFullScreen="" 
            loading="lazy"
            title="Առանձնատան տեղակայումը քարտեզի վրա"
          ></iframe>
